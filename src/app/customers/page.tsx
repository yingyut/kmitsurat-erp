...
import { customers, type Customer } from "@/lib/firestore";
...
export default function CustomersPage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setForm({ company_name: "", contact_name: "", phone: "" });
      setShowForm(false);
      await load();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="p-8">
        ...
        <div className="rounded-xl bg-card border border-border p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <input
              ...
              className="rounded-lg bg-background border border-border px-4 py-2.5 text-sm focus:outline-none focus:border-accent"
            />
            <input
      ...
      {loading ? (
        ...
        <p className="text-muted">No customers yet. Click &quot;+ Add Customer&quot; to create one.</p>
      ) : (
            ...
            <tbody>
              {list.map((c) => (
                <tr key={c.id} className="border-b border-border last:border-0 hover:bg-card-hover transition-colors">
                  <td className="px-5 py-3.5 text-sm font-medium">{c.company_name}</td>
                  <td className="px-5 py-3.5 text-sm text-muted">{c.contact_name}</td>
