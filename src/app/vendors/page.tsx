...
const empty = { name: "", contact_name: "", phone: "", email: "", address: "", notes: "", active: true, payment_terms: "", tax_id: "" };
    ...
    <div className="p-6">
      ...
      {!loading && list.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
          <button onClick={() => setActiveFilter("all")} className={`rounded-lg border p-2.5 text-left transition-colors ${activeFilter === "all" ? "border-accent bg-accent/10" : "border-border bg-card hover:bg-card-hover"}`}>
      ...
      {showForm && (
        <div className="rounded-xl bg-card border border-border p-5 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-3">
            <div><label className="text-[10px] text-muted">ชื่อบริษัท / ผู้ขาย *</label><input placeholder="เช่น บริษัท ABC จำกัด" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg bg-background border border-border px-3 py-2 text-sm focus:outline-none focus:border-accent mt-1" /></div>
            <div><label className="text-[10px] text-muted">เลขผู้เสียภาษี</label><input placeholder="13 หลัก" value={form.tax_id} onChange={e => setForm({ ...form, tax_id: e.target.value })} className="w-full rounded-lg bg-background border border-border px-3 py-2 text-sm focus:outline-none focus:border-accent mt-1" /></div>
            <div><label className="text-[10px] text-muted">เงื่อนไขชำระเงิน</label><input placeholder="เช่น เครดิต 30 วัน, เงินสด" value={form.payment_terms} onChange={e => setForm({ ...form, payment_terms: e.target.value })} className="w-full rounded-lg bg-background border border-border px-3 py-2 text-sm focus:outline-none focus:border-accent mt-1" /></div>
            <div><label className="text-[10px] text-muted">ผู้ติดต่อ</label><input placeholder="ชื่อเซลส์ที่ติดต่อ" value={form.contact_name} onChange={e => setForm({ ...form, contact_name: e.target.value })} className="w-full rounded-lg bg-background border border-border px-3 py-2 text-sm focus:outline-none focus:border-accent mt-1" /></div>
            ...
            <div className="flex items-center gap-2"><input type="checkbox" id="vendor-active" checked={form.active} onChange={e => setForm({ ...form, active: e.target.checked })} /><label htmlFor="vendor-active" className="text-sm">Active</label></div>
          </div>
        ...
        <div className="rounded-xl bg-card border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-border text-left text-xs text-muted uppercase">
              ...
              <th className="px-4 py-2.5 text-center" title="จำนวนสินค้าที่ vendor นี้ขาย">📦 สินค้า</th>
