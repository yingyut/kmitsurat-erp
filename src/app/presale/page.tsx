...
import type { PresaleRequest, Customer, Project } from "@/lib/types";
...
export default function PresalePage() {
  ...
  async function load() {
    ...
    try { const [r, c, p] = await Promise.all([fs.presaleRequests.list(), fs.customers.list(), fs.projects.list()]); setList(r); setFiltered(r); setCusts(c); setProjs(p); } catch (e) { console.error(e); } finally { setLoading(false); }
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-5"><h1 className="text-xl font-bold">Presale Requests</h1><button onClick={() => setShowForm(!showForm)} className="rounded-lg bg-accent px-4 py-2 text-sm font-medium text-white hover:bg-accent-hover">{showForm ? "Cancel" : "+ New Request"}</button></div>
      {showForm && (
